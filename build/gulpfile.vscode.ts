...
import { getMxcExcludeFilter, getRipgrepExcludeFilter } from './lib/nativeDeps.ts';
	...
	return async () => {
		...
		const patchPromises = deps.map<Promise<unknown>>(async dep => {
			...
			await rcedit(fullPath, {
				...
				'version-string': {
					'CompanyName': 'Tungsten',
					'FileDescription': product.nameLong,
					'FileVersion': packageJson.version,
					'InternalName': basename,
					'LegalCopyright': 'Copyright (C) 2026 Tungsten. Based on Visual Studio Code (MIT), Copyright (C) Microsoft Corporation.',
